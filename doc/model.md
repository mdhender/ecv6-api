...
| Deposit | `store.Deposit` | `deposit` | one row per deposit on an occupied orbit, PK `(game_id, q, r, orbit, deposit_no)`, FK `(game_id, q, r, orbit)` → `planet`; exactly one `resource` (fuel / mtls / nmtl); `initial`/`current_quantity` are positive whole numbers; `initial`/`current_yield` are **tenths of a percent** (`42` = 4.2%), always ≥ 1; current == initial at generation. Deposits live in `internal/genesis` (`GenerateDeposits`) |
| Home template deposit | `store.HomeDeposit` | `home_template_deposit` | the home-system template's deposits, PK `(game_id, orbit, deposit_no)`, FK `(game_id, orbit)` → `home_template`; same shape as Deposit, generated once (`GenerateDeposits`). The per-player copy (E2) does not touch this table |
